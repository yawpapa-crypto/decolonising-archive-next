...
function estimateUnifiedTotalCount(rankedAll) {
  ...
  for (const sectionId of ["openalex", "core", "crossref", "semantic-scholar"]) {
    const section = discoverySections[sectionId];
  ...
  const tasks = LIVE_SOURCE_ADAPTERS.map((adapter, index) => {
    return Promise.resolve()
      ...
      .then((value) => {
        ...
        if (adapterError && sectionId !== "previews" && sectionId !== "handoffs") {
...
function renderDiscoverySourceSections() {
  const renderOrder = ["openalex", "core", "crossref", "semantic-scholar", "wikidata", "openAccess", "previews"];
  return renderOrder
    .map((sectionId) => {
      const section = discoverySections[sectionId];
      const meta = DISCOVERY_SECTION_LABELS[sectionId];
