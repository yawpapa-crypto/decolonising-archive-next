...
function getResultPriority(result) {
  ...
  const access = String(result.access || "").toLowerCase();
  const source = String(result.source || "").toLowerCase();
  const institution = String(result.institution || "").toLowerCase();

  const isExternal =
    origin.includes("external") ||
    origin.includes("source") ||
    origin.includes("remote") ||
    origin.includes("search-ready") ||
    ...
    (institution &&
      institution !== "decolonising archive local index" &&
      institution !== "african philosophy working library" &&
      institution !== "local archive");

  return isExternal ? 1 : 0;
}

function orderResultsLocalFirst(results) {
  const combinedResults = Array.isArray(results) ? results : [];

  return combinedResults
    .map((item, index) => ({ item, index }))
    .sort((a, b) => {
      const priorityDiff = getResultPriority(a.item) - getResultPriority(b.item);
...
function searchLocalRecords(query) {
  const context = buildQueryContext(query);
  if (!context.raw) return orderResultsLocalFirst([...RECORDS]);

  return orderResultsLocalFirst(
    RECORDS
...
async function logSearchTerm(rawTerm){ const term = String(rawTerm || '').trim().toLowerCase().replace(/\s+/g, ' ').slice(0, 120); if (!term || term.length < 2) return; try { await fetch('/api/search-term', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ term }) }); } catch (error) { console.error('Failed to log search term:', error); } }
