 ...
 *
 * Triggers a daily digest email to all admins with daily_digest_enabled.
 *
 * Protection:
...
async function isAuthorised(req: NextRequest): Promise<boolean> {
  ...
  // Accept Vercel cron token if configured
  if (cronSecret) {
    const auth = req.headers.get("authorization") ?? "";
    if (auth === `Bearer ${cronSecret}`) return true;
  }

  // Fall back to active admin session
  try {
...
export async function POST(req: NextRequest) {
  ...
  for (const admin of admins as { id: string }[]) {
    // Only send to admins with daily_digest_enabled
    const { data: settings } = await adminClient
      .from("admin_notification_settings")
      .select("daily_digest_enabled, email_enabled")
      .eq("user_id", admin.id)
      .maybeSingle();

    const emailEnabled = (settings as { email_enabled?: boolean } | null)?.email_enabled ?? false;
    const digestEnabled = (settings as { daily_digest_enabled?: boolean } | null)?.daily_digest_enabled ?? true;
    ...
    const result = await sendAdminDigestReport(admin.id, "daily");
...
// Allow GET for easier manual testing from admin session
