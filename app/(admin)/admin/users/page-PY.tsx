...
import { deleteUserAccount, updateUserRole, updateUserStatus } from "./actions";
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((user) => (
              <div className="admin-users-row" key={user.id}>
                ...
                <form action={deleteUserAccount} className="admin-inline-form admin-delete-user-form">
                  <input type="hidden" name="user_id" value={user.id} />
                  <input
                    className="admin-filter"
                    name="confirm"
                    placeholder="Type DELETE"
                    aria-label={`Confirm deleting ${user.email ?? user.id}`}
                  />
                  <button type="submit" className="admin-small-button admin-danger-button">
                    Delete
                  </button>
                </form>
              </div>
