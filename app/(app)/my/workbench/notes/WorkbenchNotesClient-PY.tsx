  ...
  const references = useMemo(() => {
    ...
    for (const parsed of parseCitedReferencesFromHtml(editorHtml)) {
      ...
      const text =
        ...
        referenceTextFromLinkMetadata(parsed) ||
        (source ? referenceTextFromSource(source) : "") ||
        (linked ? referenceTextFromLinkedRecord(linked) : "") ||
        "";
      if (!text.trim() || isPlaceholderReferenceEntry(text)) continue;
    ...
    for (const citation of citations) {
      ...
      const text =
        referenceTextFromCitation(citation) ||
        (source ? referenceTextFromSource(source) : "") ||
        (linked ? referenceTextFromLinkedRecord(linked) : "");
      if (!text.trim() || isPlaceholderReferenceEntry(text)) continue;
      seen.add(referenceId);
      entries.push({
        ...
        recordId: citation.recordId,
        text,
        sourceUrl: citation.sourceUrl ?? source?.sourceUrl ?? null,
      });
    }
