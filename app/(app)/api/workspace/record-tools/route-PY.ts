...
export async function GET(request: NextRequest) {
  ...
  if (mode === "session") {
    ...
    let [bookmarksResult, readingListsResult, workbenchResult] = await Promise.all([
      supabase
        .from("bookmarks")
    ...
    if (isMissingReadingListGroupingError(readingListsResult.error)) {
  ...
  const supabase = await createClient();
  let [bookmarkResult, readingListsResult, submissionsResult] =
    await Promise.all([
      supabase
        .from("bookmarks")
  ...
  return NextResponse.json({
    ...
    readingLists: withReadingListGroupingDefaults(readingListsResult.data ?? []),
  ...
  if (action === "create_reading_list") {
    ...
    let listResult = await supabase
      ...
      .insert({
        ...
        group_type: clean(body.groupType) || "theme",
        group_label: clean(body.groupLabel) || null,
        is_public: Boolean(body.isPublic),
      })
      .select("id, title, description, group_type, group_label, is_public, created_at")
      .single();

    if (isMissingReadingListGroupingError(listResult.error)) {
      const fallbackListResult = await supabase
        .from("reading_lists")
        .insert({
          user_id: profile.id,
          title,
      ...
      listResult = {
        ...fallbackListResult,
        data: fallbackListResult.data
          ? withReadingListGroupingDefaults([fallbackListResult.data])[0]
          : null,
      } as typeof listResult;
    }

    const list = listResult.data;
    if (listResult.error) return jsonError(listResult.error.message, 500);
    if (!list) return jsonError("Could not create reading list.", 500);
    ...
    return NextResponse.json({
      ...
      readingList: withReadingListGroupingDefaults([list])[0],
    });
  }
