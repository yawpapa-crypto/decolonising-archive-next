...
export async function createCommunityPost(formData: FormData) {
  ...
  const spaceId = formText(formData, "space_id", 120) || null;
  ...
  if (spaceId) {
    const { data: space, error: spaceError } = await supabase
      ...
      .maybeSingle();
    if (spaceError || !space) {
      fail("/community", "That reading circle could not be found.");
    }
  }
...
export async function joinCommunitySpace(formData: FormData) {
  const profile = await requireMember("/community");
  const spaceId = formText(formData, "space_id", 120);
  const returnTo = formText(formData, "return_to", 240) || "/community/spaces";

  if (!spaceId) fail(returnTo, "Choose a reading circle to join.");

  const supabase = await createClient();
  const { error } = await supabase.from("community_space_members").upsert(
    { space_id: spaceId, user_id: profile.id, role: "member" },
  ...
  success(returnTo, "You joined the reading circle.");
}

export async function leaveCommunitySpace(formData: FormData) {
  const profile = await requireMember("/community");
  const spaceId = formText(formData, "space_id", 120);
  const returnTo = formText(formData, "return_to", 240) || "/community/spaces";

  if (!spaceId) fail(returnTo, "Choose a reading circle to leave.");
  ...
  revalidatePath("/community/spaces");
  revalidatePath(returnTo);
  success(returnTo, "You left the reading circle.");
}

export async function createCommunitySpace(formData: FormData) {
  const profile = await requireMember("/community/spaces/new");
  const name = formText(formData, "name", 80);
  const description = formText(formData, "description", 500);
  const emoji = formText(formData, "emoji", 4) || "📖";
  const slug = slugifyCommunitySpace(name);

  if (name.length < 2 || !slug) {
    fail("/community/spaces/new", "Add a name for your reading circle.");
  }
  ...
  if (error?.code === "23505") {
    fail("/community/spaces/new", "A circle with a similar name already exists. Try a different name.");
  }
  if (error || !data) {
    fail("/community/spaces/new", error?.message ?? "Could not create reading circle.");
  }

  await supabase.from("community_space_members").upsert(
    { space_id: data.id, user_id: profile.id, role: "moderator" },
    { onConflict: "space_id,user_id" },
  );

  revalidatePath("/community");
  revalidatePath("/community/spaces");
  redirect(`/community/spaces/${data.slug}?updated=${encodeURIComponent("Reading circle created.")}`);
}
