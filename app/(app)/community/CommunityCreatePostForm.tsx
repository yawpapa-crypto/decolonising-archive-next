...
import type { CommunityAttachmentOptions, CommunitySpace } from "@/src/lib/community-reading-commons";
  ...
  return (
    <details className="community-composer" id="share" open={shouldOpen}>
      ...
      <form action={createCommunityPost} className="community-form community-post-form">
        ...
        {spaces.length ? (
          <div className="community-field">
            <label htmlFor="community-post-space">Reading circle (optional)</label>
            <select id="community-post-space" name="space_id" defaultValue={defaultSpaceId}>
              <option value="">No circle — post to Commons</option>
              {spaces.map((space) => (
