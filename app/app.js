...
const LIVE_SOURCE_ADAPTERS = [
  ...
  },
  {
    ...
    trust:0.89,
    async search(query) {
      const response = await fetchWithTimeout(`/api/search/crossref?query=${encodeURIComponent(query)}&limit=8&offset=0`, {headers:{Accept:"application/json"}}, 15000);
      const json = await response.json().catch(() => ({}));
      const items = Array.isArray(json.results) ? json.results : [];
      if (!response.ok || (json.ok === false && !items.length)) return [];
      return items.map((item) => normalizeLiveRecord(item));
    }
  },
  {
    id:"loc",
