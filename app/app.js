...
const LIVE_SOURCE_ADAPTERS = [
  ...
  },
  {
    ...
    trust:0.89,
    async search(query) {
      const response = await fetchWithTimeout(`/api/search/crossref?q=${encodeURIComponent(query)}&limit=8`, {headers:{Accept:"application/json"}}, 12000);
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.ok === false) {
        throw new Error(data?.error?.message || data?.error || `Crossref search failed (${response.status})`);
      }
      return Array.isArray(data.results) ? data.results.map(item => normalizeLiveRecord(item)) : [];
    }
  },
  {
    id:"loc",
